...
class PageFactory {
	...
	/**
	 * Create the PageFactory. The construction of the Factory will already trigger the creation of the pages. They
	 * can be retrieved using the getPages() method
	 *
	 * @see #getPages()
	 *
	...
	/**
	 * This method traverses recursively the view hirarchy in order to split the component between views onto
	 * several pages.
	 *
	...
	private void splitView(View view, Shape shape) throws Exception {
        ...
        if (view.getViewCount() == 0) {
			...
			if (viewBottom <= pageStart + pageHeight) {
				...
				/*
				 * if this single view is higher than a single page, we distribute it on several pages. This is the
				 * only case where we split an atomic view.
				 */
				if (viewHeight > pageHeight) {
					while (pageEnd < viewBottom) {	//as long as we haven't put the whole view on pages ...
						int remaining = viewBottom - pageEnd;	//amount we still have to place
	...
	private void dumpViewStructure(String prefix, View view, Shape shape) {
		if (shape == null) {
			System.out.println(prefix + "# No Shape");
			return;
		}
